...
## Teamcenter 技术技巧 
**<u>1. 设置NX零件质量小数点位数</u>**
1. 修改属性映射文件，映射“TA_Mass”到Teamcenter中Double属性“g2_PartRev_EstimateWeight”
{ Datasettype="UGMASTER"
...
{ Itemtype="G2_Part"
"TA_Mass" :ItemRevision.g2_PartRev_EstimateWeight /master=cad
/description="TA_Mass"
}
}

2. 从Teamcenter打开NX，创建一个模型，在属性→权值中勾选“保存时更新数据”，保存模型分析→测量体→选择模型，勾选“关联”后点击确定。
![image1](1fad7fb89d514102ba80d34b53a098be.jpg)
3. 创建表达式TA_Mass=ug_setPartAttrValue("MASS",format("%0.2f",P16)), 设置TA_Mass属性保留两位小数点，点击绿色按钮确认。
...
设置TA_Mass属性保留两位小数点，点击绿色按钮确认。
![image3](8788841543ab43ba94d8dc138e27f82f.jpg)

**<u>2. AWC通过以下首选项来确定对一个对象进行渲染的样式表</u>**
首先，查找名称为AWC\_\[TYPE\].SUMMARYRENDERING的首选项定义的样式表。
如果不存在上面的首选项，查找名称为\[TYPE\].SUMMARYRENDERING的首选项定义的样式表。
如果上面的两个首选项都不存在，系统会根据以上的查找顺序查找父类对象类型\[TYPE\]定义的样式表,直到找到合适的渲染样式表。
