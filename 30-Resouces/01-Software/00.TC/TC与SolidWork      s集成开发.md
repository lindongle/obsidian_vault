...
aliases:
  - 1.背景介绍
linter-yaml-title-alias: 1.背景介绍
---
...
提供了几个方法（saveValidation，saveNew，saveReplace，saveInit，saveItemId），具体的可以在文件中详细查看。
2.  取消注释需要扩展的方法，例如，saveValidation专门用于 保存前校验的方法。
...
3.  编译
打开命令提示符，进入目录后执行编译：
cd C:\swim\com\transcendata\userexits
TC13/TC14 及以上版本使用以下命令： 
javac -cp "c:/swim/swim.jar;c:/swim/soa_client14.1/\*;c:/swim/soa_client14.1/external/\*;" SwimUserExits.java
...
d.部署（无需额外操作）
编译输出的.class 文件就在正确位置\<swim_install_dir\>\com\transcendata\userexits\\SWIM 启动时会自动加载，不需要重启服务或复制文件，重新启动SolidWorks/SWIM 客户端即可。
e.验证
- 启动 SolidWorks + SWIM
- 打开一个零件，执行Check In / Save操作
- 点击 Save 按钮，观察是否触发你的校验逻辑
- 如果校验失败，应弹出你自定义的提示窗口并阻止保存
