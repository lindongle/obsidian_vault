...
«span style='font-family:微软雅黑;font-size:11.0pt'»经过客户系统管理员共同研究分析，决定对于Global Pool中的“«/span»QUERY_TIMEOUT«span style='font-family:微软雅黑;font-size: 11.0pt'»”参数进行调整，将其设置为7200秒（用户会执行某些时间比较长的报表输出操作因此设置的时间比较加长，如果没有此类操作建议设置在2000秒以内）。«/span»
...
«span style='font-family:微软雅黑;font-size:11.0pt'»同时Global Pool参数可以通过修改«/span».war«span style='font-family:"Microsoft YaHei";font-size:10.0pt;background:white'»发布包«/span»«span style='font-family:微软雅黑;font-size:11.0pt'»中的«/span»globalPoolConfig.properties«span style='font-family:微软雅黑;font-size:11.0pt'»文件的内容实现。«/span»
<table>
<colgroup>
<col style="width: 100%" />
</colgroup>
...
<ul>
<li><p>USER_TIMEOUT_STATELESS</p></li>
</ul>
<blockquote>
...
<ul>
<li><p>QUERY_TIMEOUT</p></li>
</ul>
<blockquote>
<p>Configures the maximum time a server is allowed to process a single request. If this time is exceeded the server is terminated. A value of0turns off the query time-out, allowing a server to continue processing a request indefinitely.</p>
</blockquote>
<ul>
