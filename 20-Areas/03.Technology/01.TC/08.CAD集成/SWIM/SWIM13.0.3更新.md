...
在某些情况下，例如 ECAD/MCAD 数据交换和供应商协作，当前工作区可能包含不是从 Teamcenter 打开或保存到 Teamcenter 的模型。这通常是将文件从外部位置复制到工作区而没有 swim.txr 索引文件的结果。
部分或所有文件可能已作为 Teamcenter 管理的模型存在，但集成在执行保存之前不知道这一点。保存操作在 Teamcenter 中搜索其名称和类型和/或映射的关键属性值与现有 Teamcenter 管理模型相匹配的模型。如果找到匹配项，则本地模型可以保存为现有托管数据的新版本或修订版。
现在，此功能已扩展到更新模型操作，在会话中程序集上执行时。在以下示例中，所有文件都已复制到当前工作区，并且用户想要将现有的 Teamcenter 管理模型合并到他的会话中装配体中。 “更新模型”对话框指示在 Teamcenter 中找到了哪些模型，并自动选择它们进行更新，从而轻松地使本地装配体保持最新状态。
