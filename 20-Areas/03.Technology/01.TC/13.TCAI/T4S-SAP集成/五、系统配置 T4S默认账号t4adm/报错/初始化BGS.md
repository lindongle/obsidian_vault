...
<span style='color:#333333'>在相应的BGS停止后，您可以重新开始初始化BGS：</span>
1.  使用文本编辑器打开<span style='font-weight:bold;background:#F2F2F2'>\<BGS_ROOT</span><span style='background:#F2F2F2'>\>/var/conf/uuid</span>，并记住其中的旧 UUID。后续步骤将需要它。
2.  <span style='color:#333333'>从操作系统密码管理器中删除条目**Siemens_PL4x\_\<uuid\>/internal/token**和**Siemens_PL4x\_\<uuid\>/internal/InitialKey**。</span>
3.  删除BGS目录下的<span style='background:#F2F2F2'>\<</span>«span style='font-weight:bold;background: \#F2F2F2'»BGS_ROOT«/span»<span style='background:#F2F2F2'>\>/var/conf/uuid</span>和<span style='font-weight:bold;background:#F2F2F2'>\<BGS_ROOT</span><span style='background:#F2F2F2'>\>/var/db/structured_secrets.db</span>文件。
...
<span style='color:#FA0000'>未出现自动关闭后无法启动BGS的问题，忽略以上。</span>
